...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-gray-600 mb-2">
            ...
            <span class="text-gray-900">Edit Client</span>
        </div>
        <h2 class="text-2xl font-bold text-gray-800">Edit Client</h2>
    </div>

    <!-- Form -->
    <form action="{{ route('ourclient.update', $ourclient) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
                ...
                <div>
                    ...
                    <div class="flex items-center gap-4">
                        <div id="image-preview" class="w-32 h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50 overflow-hidden">
                            @if($ourclient->image)
                                <img src="{{ asset('storage/' . $ourclient->image) }}" 
                        ...
                        <div class="flex-1">
                            ...
                            <p class="mt-1 text-xs text-gray-500">Leave empty to keep current image. Max 2MB</p>
                            @error('image')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
